    ...
    if (localSystem?.testUrl !== null) {
      if (!req.url.includes('api')) {
        monitoredSystems.unshift({
          key: 'local index',
          required: true,
          getStatus: urlCheck(`http://localhost:${req.socket.localPort}${req.url.replace('/_monitor', '')}`),
        })
      }
      if (req.url.includes('api')) {
        monitoredSystems.unshift({
          ...
          required: true,
          getStatus: urlCheck(`http://localhost:${req.socket.localPort}${req.url.replace('/_monitor', '/swagger/')}`),
        })
      }
    }
  }
