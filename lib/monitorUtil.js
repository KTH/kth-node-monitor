...
function localUrlCheck(req, url) {
  ...
  return async function (system) {
    ...
    const res = await new Promise(resolve =>
      protocols[protocol]
        .get(`${protocol}://${host}:${port}${url}`, res => res.on('data', () => {}).on('end', () => resolve(res)))
    ...
    return {
      ...
      isResolved: true,
      message: `- ${system.key}: ${
        res.statusCode == 200 ? 'OK' : 'ERROR'
      } | Response time: ${dur}ms | Required to work: true`,
    }
  }
}
...
async function monitorSystems(req, res, options, monitoredSystems) {
  ...
  if (!req.originalUrl.includes('api') && !options?.noIndexCheck) {
    monitoredSystems.unshift({
      key: 'local index',
      required: true,
      getStatus: localUrlCheck(req, req.originalUrl.replace(/\/_monitor\/?$/, '')),
    })
  }
  if (req.originalUrl.includes('api') && !options?.noSwaggerCheck) {
    monitoredSystems.unshift({
      key: 'local swagger api',
      required: true,
      getStatus: localUrlCheck(req, req.originalUrl.replace(/\/_monitor\/?$/, '/swagger')),
    })
  }
