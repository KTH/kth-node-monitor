...
function localUrlCheck(req, url) {
  ...
  return async function (system) {
    ...
    const res = await new Promise(resolve =>
      protocols[protocol]
        .get(`${protocol}://${host}:${port}${url}`, obj => obj.on('data', () => {}).on('end', () => resolve(obj)))
...
async function monitorSystems(req, res, options, monitoredSystems) {
  ...
  if (!req.url.includes('api') && !options?.noIndexCheck) {
    monitoredSystems.unshift({
      key: 'local index',
      required: true,
      getStatus: localUrlCheck(req, req.url.replace(/\/_monitor\/?$/, '')),
    })
  }
  if (req.url.includes('api') && !options?.noSwaggerCheck) {
    monitoredSystems.unshift({
      ...
      required: true,
      getStatus: localUrlCheck(req, req.url.replace(/\/_monitor\/?$/, '/swagger')),
    })
  }
  // console.log(monitoredSystems)
