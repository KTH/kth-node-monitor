...
const useLocalSwagger = process.env.USE_LOCAL_SWAGGER === 'true'
...
async function monitorSystems(req, res, monitoredSystems) {
  if (req?.url) {
    ...
    const isLocalHost = /localhost/.test(req.hostname) || /^api$/.test(req.hostname)
    const makeLocalUrl = (req, testUrl, path = '') =>
      `${`http${!isLocalHost ? 's' : ''}`}://${req.hostname}${isLocalHost ? `:${req.socket.localPort}` : ''}${
        testUrl || req.url.substring(0, req.url.indexOf('/_monitor'))
      }${path}`

    if (localSystem?.testUrl !== null) {
      if (!req.url.includes('api')) {
        monitoredSystems.unshift({
          key: 'local index',
          required: true,
          getStatus: urlCheck(makeLocalUrl(req, localSystem?.testUrl, '')),
        })
      }
      if (req.url.includes('api')) {
        monitoredSystems.unshift({
          ...
          required: true,
          getStatus: useLocalSwagger
            ? urlCheck(`http://localhost:${req.socket.localPort}${req.url.replace('/_monitor', '/swagger/')}`)
            : urlCheck(makeLocalUrl(req, localSystem?.testUrl, '/swagger/')),
        })
      }
    }
  }
