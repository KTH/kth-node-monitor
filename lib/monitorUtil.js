...
const makeLocalUrl = (req, path, testUrl) => {
  const host = `http://localhost:${req.socket.localPort}`
  if (testUrl) {
    // testUrl should not include hostname, port etcetera
    return `${host}${testUrl}`
  }
  return `${host}${req.url.split('/_monitor')[0]}${path}`
}
...
async function monitorSystems(req, res, monitoredSystems) {
  if (req?.url) {
    ...
    // The URL check can be disabled by setting testUrl to null
    // If the testUrl is undefined, test localhost index or swagger
    if (localSystem?.testUrl !== null) {
      ...
      if (req.url.includes('api')) {
        monitoredSystems.unshift({
          ...
          required: true,
          getStatus: urlCheck(makeLocalUrl(req, '/swagger/', localSystem?.testUrl)),
        })
      }
    }
  }
