...
describe('System Response', () => {
  it('Get a 501 Internal Server Error when a check is not configuration properly.', () => {
    const responseObject = httpResponse.configurationError(interfaces.names.KTH_NODE_MONGODB, {
      required: true
    })
  ...
  it('Get a OK message when check worked, and required.', () => {
    const responseObject = systemResponse.works(interfaces.names.KTH_NODE_MONGODB, {
      required: true
    })
  ...
  it("Get a 'The application can still function' message when check faild, and required.", () => {
    const responseObject = systemResponse.failed(interfaces.names.KTH_NODE_MONGODB, {
      required: true
    })
