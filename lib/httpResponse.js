...
const isOk = statusCode => equalStatusCode(statusCodes.OK, statusCode)

const isBadGateway = statusCode => equalStatusCode(statusCodes.BAD_GATEWAY, statusCode)

const isInternalServerError = statusCode => equalStatusCode(statusCodes.INTERNAL_SERVER_ERROR, statusCode)
...
const getRequestTimeMs = requestStarted => Date.now() - requestStarted
...
const works = (key, options, requestTimeMs) => getResponse(key, statusCodes.OK, options, requestTimeMs)

const error = (key, options, requestTimeMs) => getResponse(key, statusCodes.BAD_GATEWAY, options, requestTimeMs)

const failed = (key, options, requestTimeMs) =>
  getResponse(key, statusCodes.SERVICE_UNAVAILABLE, options, requestTimeMs)

const configurationError = (key, options) => getResponse(key, statusCodes.INTERNAL_SERVER_ERROR, options, -1)
...
const getMessage = (key, statusCode, required, responseTime) => {
  ...
  if (isBadGateway(statusCode)) {
    ...
    prefix = `- ${key}: ERROR - Connected but got an unexpected http response '${statusCode}' from service | Response time: ${responseTime}ms |`
  }
...
const getUniqueReferens = () => Math.random().toString(36).substring(2)
