...
function getMessage(key, statusCode, required, responseTime) {
  ...
  if (isBadGateway(statusCode)) {
    ...
    prefix = `- ${key}: ERROR - Connected but got an unexpected response from service | Response time ${responseTime}ms | Required to work: ${required} |`;
  }

  if (required) {
    return `${prefix} Has to work for the APPLICATION_STATUS to say OK`;
  }
  return `${prefix} The application can still function without this service`;
}
