...
function getMessage(key, statusCode, required, responseTime) {
  ...
  if (isBadGateway(statusCode)) {
    prefix = `- ${key}: Unable to connect | Response time ${responseTime}ms | The service is probably down or it is a networking issue.`;
  } else {
    prefix = `- ${key}: Unexpected response from service | Response time ${responseTime}ms | Required to work: ${required} |`;
  }

  if (required) {
    return `${prefix} This service has to work for the APPLICATION_STATUS to say OK.`;
  }
  return `${prefix} The application can still function without this service functioning properly."`;
}
