  ...
  status: function(client, key, endpointPath, options) {
    if (endpointPath === undefined) {
      ...
      return Promise.resolve(
        _createApiStatusObj(key, 400, options && options.required)
      );
    } else {
      ...
      if (client.getAsync) {
        ...
        return client
          ...
          .catch(err => {
            log.error(
              { err: err },
              `Cught an unexpected error when checking [1] '${endpointPath}'.`
            );
            const statusObj = _createApiStatusObj(
              key,
              503,
        ...
        return client
          ...
          .catch(err => {
            log.error(
              { err: err },
              `Cught an unexpected error when checking [2] '${endpointPath}'.`
            );
            const statusObj = _createApiStatusObj(
              key,
              503,
...
createUtility({
  ...
  status: function(endpoint, options) {
    ...
    if (endpointBaseUri === undefined) {
      // We couldn't resolve the endpoint
      return Promise.resolve(
        _createApiStatusObj(endpoint.key, 400, options && options.required)
      );
    } else if (
      ...
      return endpoint.client
        ...
        .catch(err => {
          log.error(
            { err: err },
            `Cught an unexpected error when checking [3] '${endpointBaseUri}'.`
          );
          const statusObj = _createApiStatusObj(
            endpoint.key,
            503,
...
createUtility({
  ...
  status: function(ldap, options) {
    if (options && options.testSearch) {
      ...
      if (ldap.isOk()) {
        return Promise.resolve(
          _createApiStatusObj("ldap", 200, options && options.required)
        );
      } else {
        return Promise.resolve(
          _createApiStatusObj("ldap", 503, options && options.required)
        );
      }
    }
  }
...
createUtility({
  ...
  status: function(db, options) {
    if (db.isOk()) {
      return Promise.resolve(
        _createApiStatusObj("mongodb", 200, options && options.required)
      );
    } else {
      return Promise.resolve(
        _createApiStatusObj("mongodb", 503, options && options.required)
      );
    }
  }
}).registerWith(registry);
...
createUtility({
  ...
  status: function(db, options) {
    return db
      ...
      .then(() => {
        return Promise.resolve(
          _createApiStatusObj("sqldb", 200, options && options.required)
        );
      })
      .catch(err => {
        log.error(
        ...
        return Promise.resolve(
          _createApiStatusObj("sqldb", 503, options && options.required)
        );
      });
  }
}).registerWith(registry);
    ...
    return redis("HealthCheck", redisOptions).then(function(client) {
      return new Promise(function(resolve, reject) {
        client.ping(function(err, val) {
          ...
          if (err) {
            return resolve(
              _createApiStatusObj("redis", 503, options && options.required)
            );
          } else {
            return resolve(
              _createApiStatusObj("redis", 200, options && options.required)
            );
          }
        });
      });
        ...
        var systemOk = Object.keys(subSystems).reduce((systemOk, apiKey) => {
          return (
            ...
            (subSystems[apiKey].required
              ? subSystems[apiKey].statusCode === 200
              : true)
          );
