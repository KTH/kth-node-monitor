  ...
  status: function (endpoint, options) {
    ...
    if (endpointBaseUri === undefined) {
      ...
      .catch(err => {
          if (err != null) {
            console.log(
              `Got an unhandled exception when checking _monitor: '${err}'.`
            );
          } else {
            console.log(
              `Unknown error catched when checking _monitor.`
            );
          }
...
createUtility({
  ...
  status: function (ldap, options) {
    if (options && options.testSearch) {
      return ldap.testSearch(options)
        .then(res => {
          if (res.isOk) {
        ...
        .catch(err => {
          log.error({err: err}, '[kth-node-ldap] There was an error when resolving ldap status')
          return Promise.resolve({ key: 'ldap', statusCode: 503, message: `- ldap: ERROR (${err.message})` })
        })
    } else {
      if (ldap.isOk()) {
...
createUtility({
  ...
  status: function (localSystems, subSystems) {
    ...
    return Promise.all(subSystems)
      ...
      .catch(err => {
        log.error({err: err}, '[kth-node-system-check] There was an error when resolving application status')
        return {
          statusCode: 503,
          message: 'ERROR: ' + (err.message || '[kth-node-system-check] Unknown error'),
          subSystems: subSystems,
          localSystems: localSystems
        }
      })
  ...
  renderText: function (systemHealth) {
    if (systemHealth.message != 'OK') {
      console.log(`Error checking _monitor. Application status is '${systemHealth}'.`)
    }
