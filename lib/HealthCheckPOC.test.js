...
const os = require('os')
os.hostname = jest.fn().mockReturnValue('test-host')

const ONE_MEGABYTE = 1024 * 1024
process.memoryUsage = jest.fn().mockReturnValue({
  rss: 79 * ONE_MEGABYTE,
  heapTotal: 80 * ONE_MEGABYTE,
  heapUsed: 81 * ONE_MEGABYTE,
  external: 82 * ONE_MEGABYTE,
})
...
function runTestsAboutSupportedSubSystems() {
  ...
  describe.each(setups)('- in combination with sub-system "%s"', (name, suffix, dataList) => {
    ...
    testIncludingSubSystem({ suffix, dataList, mode: 'Argument' })
...
function testIncludingSubSystem({ suffix, dataList, mode }) {
  ...
  it(`when only including sub-system (mode "${mode}") - succeeds${lengthText}`, () => {
    dataList.forEach(dataItem => {
      const systemStatus = new HealthCheck()
      if (mode === 'Argument') {
        systemStatus[`include${suffix}`](dataItem, true)
      } else {
